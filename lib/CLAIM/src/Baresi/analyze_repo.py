...
with open('src/Baresi/consts/db.csv') as db_file:
    dbs = [db.lower() for db in db_file.read().splitlines()]
with open('src/Baresi/consts/bus.csv') as bus_file:
    buses = [bus.lower() for bus in bus_file.read().splitlines()]
with open('src/Baresi/consts/lang.csv') as lang_file:
    langs = [lang.lower() for lang in lang_file.read().splitlines()]
with open('src/Baresi/consts/server.csv') as server_file:
    servers = [server.lower() for server in server_file.read().splitlines()]
with open('src/Baresi/consts/gateway.csv') as gate_file:
    gates = [gate.lower() for gate in gate_file.read().splitlines()]
with open('src/Baresi/consts/monitor.csv') as monitor_file:
    monitors = [monitor.lower() for monitor in monitor_file.read().splitlines()]
with open('src/Baresi/consts/discovery.csv') as disco_file:
...
def analyze_repo(url):
    ...
    try:
        with lock:
            ...
            outfile = path.join('../../data/results', analysis['name'].replace('/', '#'))
